...
def test_compare_fields_unit_cases():
    """Unit tests for the optimized _compare_fields function."""
    from syntagmax.change_diff import _compare_fields

    # No changes
    base = {'status': ['active', 'draft'], 'priority': 'high'}
    target = {'status': ['draft', 'active'], 'priority': 'high'}
    assert _compare_fields(base, target) == {}

    # Field added
    base = {'priority': 'high'}
    target = {'status': 'active', 'priority': 'high'}
    assert _compare_fields(base, target) == {'status': (None, 'active')}

    # Field removed
    base = {'status': 'active', 'priority': 'high'}
    target = {'priority': 'high'}
    assert _compare_fields(base, target) == {'status': ('active', None)}

    # Field modified (different lengths of lists)
    base = {'status': ['active', 'draft']}
    target = {'status': ['active']}
    assert _compare_fields(base, target) == {'status': (['active', 'draft'], ['active'])}

    # Field modified (same length list, different values)
    base = {'status': ['active', 'draft', 1]}
    target = {'status': ['active', 'retired', 1]}
    assert _compare_fields(base, target) == {'status': (['active', 'draft', 1], ['active', 'retired', 1])}

    # Same length, same items but different types (e.g. str vs int)
    base = {'status': ['1', '2']}
    target = {'status': [1, 2]}
    # Under map(str, ...), ["1", "2"] sorted is ["1", "2"], [1, 2] sorted and mapped to str is also ["1", "2"].
    # However, Python's list inequality (base_val != target_val) will first trigger and detect they are different,
    # then check sorted(map(str, base_val)) != sorted(map(str, target_val)). Since those stringified elements are equal,
    # it won't mark them as changed. This preserves original behaviour.
    assert _compare_fields(base, target) == {}
