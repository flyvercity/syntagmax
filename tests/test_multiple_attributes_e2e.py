...
def input_record(tmp_path):
    ...
    return InputRecord(name='test', dir='.', record_base=tmp_path, filepaths=[], driver='text', default_atype='REQ', marker='REQ')
...
def test_obsidian_extractor_comma_separated_yaml(config, input_record, metamodel, tmp_path):
    ...
    local_metamodel = copy.deepcopy(metamodel)

    local_metamodel['artifacts']['REQ']['attributes']['parent'] = {
        'name': 'parent',
        'presence': 'optional',
        'multiple': True,
        'type_info': {'type': 'reference'},
    }
    ...
    extractor = ObsidianExtractor(config, input_record, metamodel=local_metamodel)
    ...
    assert artifact.fields['parent'] == ['REQ-001', 'REQ-002']
