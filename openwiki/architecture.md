...
description: High-level architecture of the Syntagmax codebase, including runtime shape, analysis and publish pipelines, configuration, and plugin system.
tags: [architecture, pipeline, plugins]
---
...
## Runtime shape
- `src/syntagmax/cli.py` defines the Click command group and the `init`, `analyze`, `publish`, `edit`, `ai`, and `mcp` command surfaces.
...
### Task generation
When enabled via `--tasks` flag or configuration, the pipeline can automatically generate impact analysis tasks from the artifact graph. These tasks are written to the configured tasks directory and can be used for further verification or documentation.
...
7. The CLI writes `.md` files to `.syntagmax/outputs/` (formerly `.syntagmax/reports/`), then optionally converts to `.docx` / `.pdf` via `pandoc.py`.
