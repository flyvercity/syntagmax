...
2. Add a per-section `attribute_presence` field on `TableSection` and `TextSection` (optional, overrides the global when set).
...
5. For text sections: attributes without values are skipped (empty bold label with no text is not useful).
...
7. Edge case: if no metamodel is available for an artifact type, `mandatory` degrades to `values-only`; `all` renders all listed attributes regardless.
...
5. Modify the table and text rendering loops in `render_block` to apply the presence filter.

### Configuration Example (YAML)

```yaml
start_level: 1
attribute_presence: all

render:
  REQ:
    - type: table
      attribute_presence: values-only
      attributes:
        - id:
            alias: "Identifier"
        - parent:
...
1. If the section (`TableSection`/`TextSection`) has an explicit `attribute_presence` value, use it.
...
**Objective:** Extend `PublishConfig`, `TableSection`, and `TextSection` with the new setting.

**Implementation guidance:**
- In `src/syntagmax/publish_config.py`:
  - Define `AttributePresence = Literal['all', 'mandatory', 'values-only']` at module level.
  - Add `attribute_presence: AttributePresence = Field(default='all')` to `PublishConfig`.
  - Add `attribute_presence: AttributePresence | None = Field(default=None)` to `TableSection` and `TextSection`.

**Test requirements:**
- Unit test: `PublishConfig()` has `attribute_presence == 'all'`.
- Unit test: `PublishConfig.model_validate({'attribute_presence': 'mandatory'})` parses correctly.
- Unit test: `PublishConfig.model_validate({'attribute_presence': 'invalid'})` raises `ValidationError`.
- Unit test: `TableSection` with `attribute_presence: 'values-only'` parses correctly.
- Unit test: `TableSection` without `attribute_presence` has `None`.
- Unit test: YAML and TOML files with `attribute_presence` at both levels parse correctly.

**Demo:** `PublishConfig.model_validate({'attribute_presence': 'mandatory'}).attribute_presence == 'mandatory'`

### Task 2: Implement metamodel condition evaluator

**Objective:** Create a helper function that determines whether a named attribute is mandatory for a given artifact, evaluating conditional rules against the artifact's field values.

**Implementation guidance:**
- In `src/syntagmax/publish.py`, add:
  - `_is_anchor_truthy(anchor_value, anchor_name: str, atype: str, metamodel: dict) -> bool` — resolves truthiness using metamodel's custom boolean definitions when available.
  - `is_attribute_mandatory(attr_name: str, atype: str, artifact: Artifact, metamodel: dict | None) -> bool` — iterates rules, evaluates conditions, returns True if any mandatory rule applies.
- For `_is_anchor_truthy`:
...
**Demo:** `is_attribute_mandatory('parent', 'SRS', artifact_with_derive_false, metamodel)` returns `True`.
...
- In `publish.py`, in the `isinstance(sec, TableSection)` branch of `render_block`:
  ...
  - For each attribute in `sec.attributes`:
    ...
    - Apply `should_render_attribute(attr_name, val, presence_mode, atype, artifact, metamodel)`.
    - If should render: append row with `val or ''` (empty cell for None).
    - If should not render: skip.
  - Access metamodel via `context.config.metamodel` if context is available; otherwise default to `values-only` behavior (no metamodel = can't evaluate).
...
- Unit test: no context (metamodel unavailable) with `mandatory` mode degrades to `values-only`.
...
### Task 4: Implement presence filtering in text section rendering

**Objective:** Modify the `TextSection` rendering loop in `render_block` to apply the attribute presence filter for text sections.

**Implementation guidance:**
- In `publish.py`, in the `isinstance(sec, TextSection)` branch of `render_block`:
  - Resolve effective presence the same way as table sections.
  - For text sections, the filter outcome only matters for deciding whether to skip an attribute entirely. Since empty-value text renders are not useful, the effective behavior is:
    - `all` / `mandatory`: same as `values-only` for text — skip if no value (no point rendering `**Label**\n\n\n\n`).
  - In other words, text sections always skip attributes without values regardless of `attribute_presence`. The setting has no visible effect on text sections.
  - Still, the field should be accepted on `TextSection` for forward compatibility and config uniformity.

**Test requirements:**
- Unit test: text section with `attribute_presence: all` still skips attributes without values.
- Unit test: text section with values renders normally regardless of mode.

**Demo:** Text sections behave identically across all three modes (only render attributes with values).
...
- Update `docs/reference/publishing.md`:
  - Add `attribute_presence` to the Global Parameters table with type `string`, default `"all"`, and description.
  - In the Render Section documentation, document the per-section `attribute_presence` override on both `table` and `text` section types.
  - Add a note explaining that text sections always skip empty-value attributes regardless of mode.
  - Update the Full Schema (YAML) and Full Schema (TOML) examples to include `attribute_presence`.
  - Add a subsection explaining condition evaluation for `mandatory` mode.

**Test requirements:** N/A (documentation only).

**Demo:** Documentation clearly describes global and per-section `attribute_presence` with examples and condition evaluation semantics.
